package com.tencent.mm.plugin.card.ui;

import android.content.Context;
import android.content.Intent;
import android.content.res.Resources;
import android.net.Uri;
import android.os.Bundle;
import android.text.TextUtils;
import android.view.LayoutInflater;
import android.view.MenuItem;
import android.view.MenuItem.OnMenuItemClickListener;
import android.view.View;
import android.widget.TextView;
import com.tencent.gmtrace.GMTrace;
import com.tencent.mm.R.e;
import com.tencent.mm.R.h;
import com.tencent.mm.R.i;
import com.tencent.mm.R.l;
import com.tencent.mm.R.o;
import com.tencent.mm.plugin.card.b.l;
import com.tencent.mm.plugin.card.widget.CardTextPreference;
import com.tencent.mm.protocal.c.jf;
import com.tencent.mm.protocal.c.jn;
import com.tencent.mm.protocal.c.nc;
import com.tencent.mm.sdk.platformtools.w;
import com.tencent.mm.ui.base.preference.MMPreference;
import com.tencent.mm.ui.base.preference.Preference;
import com.tencent.mm.ui.base.preference.PreferenceSmallCategory;
import com.tencent.mm.ui.base.preference.PreferenceTitleCategory;
import com.tencent.mm.ui.base.preference.f;
import java.util.ArrayList;
import java.util.Collection;
import java.util.LinkedList;
import java.util.List;

public class CardDetailPreference
  extends MMPreference
{
  private final String TAG;
  f jqf;
  private com.tencent.mm.plugin.card.base.b kmJ;
  private List<com.tencent.mm.plugin.card.model.b> kmN;
  private TextView ktQ;
  
  public CardDetailPreference()
  {
    GMTrace.i(4957600219136L, 36937);
    this.TAG = "MicroMsg.CardDetailPreference";
    this.kmN = new ArrayList();
    GMTrace.o(4957600219136L, 36937);
  }
  
  private void aku()
  {
    GMTrace.i(4958002872320L, 36940);
    int i = 0;
    while (i < this.kmN.size())
    {
      com.tencent.mm.plugin.card.model.b localb = (com.tencent.mm.plugin.card.model.b)this.kmN.get(i);
      Preference localPreference = new Preference(this);
      localPreference.setLayoutResource(R.i.dap);
      localPreference.setKey(localb.title);
      localPreference.setTitle(localb.title);
      if (!TextUtils.isEmpty(localb.kns)) {
        localPreference.setSummary(localb.kns);
      }
      if (localb.knE) {
        akw();
      }
      this.jqf.a(localPreference);
      i += 1;
    }
    GMTrace.o(4958002872320L, 36940);
  }
  
  private void akv()
  {
    GMTrace.i(4958137090048L, 36941);
    if ((this.kmJ.aiK().tEX != null) && (this.kmJ.aiK().tEX != null))
    {
      int i = 0;
      while (i < this.kmJ.aiK().tEX.size())
      {
        nc localnc = (nc)this.kmJ.aiK().tEX.get(i);
        if (!TextUtils.isEmpty(localnc.kns))
        {
          CardTextPreference localCardTextPreference = new CardTextPreference(this);
          localCardTextPreference.setLayoutResource(R.i.dap);
          localCardTextPreference.setTitle(localnc.kns);
          localCardTextPreference.amp();
          tV(localnc.title);
          this.jqf.a(localCardTextPreference);
        }
        i += 1;
      }
      akw();
    }
    GMTrace.o(4958137090048L, 36941);
  }
  
  private void akw()
  {
    GMTrace.i(4958271307776L, 36942);
    PreferenceSmallCategory localPreferenceSmallCategory = new PreferenceSmallCategory(this);
    this.jqf.a(localPreferenceSmallCategory);
    GMTrace.o(4958271307776L, 36942);
  }
  
  private void tV(String paramString)
  {
    GMTrace.i(4958405525504L, 36943);
    PreferenceTitleCategory localPreferenceTitleCategory = new PreferenceTitleCategory(this);
    if (TextUtils.isEmpty(paramString)) {
      localPreferenceTitleCategory.setTitle(" ");
    }
    for (;;)
    {
      this.jqf.a(localPreferenceTitleCategory);
      GMTrace.o(4958405525504L, 36943);
      return;
      localPreferenceTitleCategory.setTitle(paramString);
    }
  }
  
  protected final void MH()
  {
    GMTrace.i(4957868654592L, 36939);
    this.kmJ = ((com.tencent.mm.plugin.card.base.b)getIntent().getParcelableExtra("key_card_info_data"));
    if ((this.kmJ == null) || (this.kmJ.aiK() == null) || (this.kmJ.aiL() == null))
    {
      w.e("MicroMsg.CardDetailPreference", "mCardInfo == null or mCardInfo.getCardTpInfo() == null or mCardInfo.getDataInfo() == null");
      finish();
      GMTrace.o(4957868654592L, 36939);
      return;
    }
    Object localObject = new StringBuilder();
    if (!TextUtils.isEmpty(this.kmJ.aiK().koC)) {
      ((StringBuilder)localObject).append(this.kmJ.aiK().koC);
    }
    ((StringBuilder)localObject).append(getString(R.l.dFe));
    rV(((StringBuilder)localObject).toString());
    a(new MenuItem.OnMenuItemClickListener()
    {
      public final boolean onMenuItemClick(MenuItem paramAnonymousMenuItem)
      {
        GMTrace.i(5030480445440L, 37480);
        CardDetailPreference.this.finish();
        GMTrace.o(5030480445440L, 37480);
        return true;
      }
    });
    this.jqf = this.vOv;
    this.kmN.clear();
    if ((this.kmJ.aiL().tEp != null) && (this.kmJ.aiL().tEp.size() > 0)) {
      this.kmN.addAll(l.aa(this.kmJ.aiL().tEp));
    }
    if ((this.kmJ.aiL().tEq != null) && (this.kmJ.aiL().tEq.size() > 0))
    {
      localObject = l.aa(this.kmJ.aiL().tEq);
      ((com.tencent.mm.plugin.card.model.b)((List)localObject).get(0)).knE = true;
      this.kmN.addAll((Collection)localObject);
    }
    akw();
    akv();
    aku();
    if (!TextUtils.isEmpty(this.kmJ.aiK().tEY))
    {
      localObject = new CardTextPreference(this);
      ((CardTextPreference)localObject).setLayoutResource(R.i.dap);
      ((CardTextPreference)localObject).setTitle(getString(R.l.dFV));
      ((CardTextPreference)localObject).setKey("key_pic_detail");
      ((CardTextPreference)localObject).amp();
      akw();
      this.jqf.a((Preference)localObject);
    }
    if (!TextUtils.isEmpty(this.kmJ.aiK().tEV))
    {
      akw();
      localObject = new CardTextPreference(this);
      ((CardTextPreference)localObject).setLayoutResource(R.i.dap);
      ((CardTextPreference)localObject).setKey("card_phone");
      ((CardTextPreference)localObject).setTitle(R.l.enZ);
      ((CardTextPreference)localObject).setSummary(this.kmJ.aiK().tEV);
      ((CardTextPreference)localObject).kBg = getResources().getColor(R.e.aOU);
      ((CardTextPreference)localObject).amo();
      this.jqf.a((Preference)localObject);
      tV("");
    }
    if ((this.kmJ.aiK() != null) && (!TextUtils.isEmpty(this.kmJ.aiK().aEn)) && (this.ktQ != null))
    {
      this.ktQ.setText(this.kmJ.aiK().aEn);
      this.ktQ.setVisibility(0);
    }
    GMTrace.o(4957868654592L, 36939);
  }
  
  public final int QD()
  {
    GMTrace.i(4958539743232L, 36944);
    int i = R.o.fhj;
    GMTrace.o(4958539743232L, 36944);
    return i;
  }
  
  public final boolean a(f paramf, Preference paramPreference)
  {
    GMTrace.i(4958673960960L, 36945);
    if (paramPreference.iay.equals("card_phone"))
    {
      paramf = this.kmJ.aiK().tEV;
      Intent localIntent = new Intent("android.intent.action.DIAL");
      localIntent.setFlags(268435456);
      localIntent.setData(Uri.parse("tel:" + paramf));
      startActivity(localIntent);
    }
    if (paramPreference.iay.equals("key_pic_detail")) {
      com.tencent.mm.plugin.card.b.b.a(this, this.kmJ.aiK().tEY, 0);
    }
    for (;;)
    {
      GMTrace.o(4958673960960L, 36945);
      return false;
      int i = 0;
      while (i < this.kmN.size())
      {
        paramf = (com.tencent.mm.plugin.card.model.b)this.kmN.get(i);
        if ((paramf.title != null) && (paramf.title.equals(paramPreference.iay)) && (!TextUtils.isEmpty(paramf.url)))
        {
          if (com.tencent.mm.plugin.card.b.b.c(this.kmJ.aiO(), paramf.tEL, paramf.tEM, 1028, 0))
          {
            GMTrace.o(4958673960960L, 36945);
            return false;
          }
          com.tencent.mm.plugin.card.b.b.a(this, paramf.url, 1);
        }
        i += 1;
      }
    }
  }
  
  public final View akx()
  {
    GMTrace.i(4958808178688L, 36946);
    View localView = getLayoutInflater().inflate(R.i.dgN, null);
    this.ktQ = ((TextView)localView.findViewById(R.h.cyK));
    this.ktQ.setVisibility(8);
    GMTrace.o(4958808178688L, 36946);
    return localView;
  }
  
  public void onCreate(Bundle paramBundle)
  {
    GMTrace.i(4957734436864L, 36938);
    super.onCreate(paramBundle);
    MH();
    GMTrace.o(4957734436864L, 36938);
  }
}


/* Location:              /Users/xianghongyan/decompile/dex2jar/classes2-dex2jar.jar!/com/tencent/mm/plugin/card/ui/CardDetailPreference.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */