package com.tencent.mm.plugin.collect.reward.a;

import android.app.Activity;
import android.content.DialogInterface;
import android.content.DialogInterface.OnClickListener;
import com.tencent.gmtrace.GMTrace;
import com.tencent.mm.R.l;
import com.tencent.mm.ac.k;
import com.tencent.mm.network.j;
import com.tencent.mm.network.p;
import com.tencent.mm.ui.base.h;
import java.lang.ref.WeakReference;

public abstract class a
  extends k
  implements j
{
  protected int errCode;
  protected int errType;
  protected String ftU;
  protected WeakReference<Activity> kLa;
  protected boolean kLb;
  protected boolean kLc;
  
  public a()
  {
    GMTrace.i(19349632974848L, 144166);
    this.errCode = 0;
    this.errType = 0;
    this.kLb = false;
    this.kLc = false;
    GMTrace.o(19349632974848L, 144166);
  }
  
  public final a a(a parama)
  {
    GMTrace.i(19350304063488L, 144171);
    if ((!this.kLb) && (!this.kLc)) {
      parama.i(this);
    }
    GMTrace.o(19350304063488L, 144171);
    return this;
  }
  
  public final void a(int paramInt1, int paramInt2, int paramInt3, String paramString, p paramp, byte[] paramArrayOfByte)
  {
    GMTrace.i(19350169845760L, 144170);
    if ((paramInt2 != 0) || (paramInt3 != 0))
    {
      this.kLb = true;
      if ((anL()) && (this.kLa != null))
      {
        paramArrayOfByte = (Activity)this.kLa.get();
        if (paramArrayOfByte != null) {
          h.a(paramArrayOfByte, paramArrayOfByte.getString(R.l.eZz), null, false, new DialogInterface.OnClickListener()
          {
            public final void onClick(DialogInterface paramAnonymousDialogInterface, int paramAnonymousInt)
            {
              GMTrace.i(19347485491200L, 144150);
              GMTrace.o(19347485491200L, 144150);
            }
          });
        }
      }
    }
    this.errCode = paramInt3;
    this.errType = paramInt2;
    this.ftU = paramString;
    b(paramInt2, paramInt3, paramString, paramp);
    GMTrace.o(19350169845760L, 144170);
  }
  
  protected boolean anL()
  {
    GMTrace.i(19349767192576L, 144167);
    GMTrace.o(19349767192576L, 144167);
    return true;
  }
  
  public final a b(a parama)
  {
    GMTrace.i(19350438281216L, 144172);
    if (this.kLc) {
      parama.i(this);
    }
    GMTrace.o(19350438281216L, 144172);
    return this;
  }
  
  protected abstract void b(int paramInt1, int paramInt2, String paramString, p paramp);
  
  public final a c(a parama)
  {
    GMTrace.i(19350572498944L, 144173);
    if (this.kLb) {
      parama.i(this);
    }
    GMTrace.o(19350572498944L, 144173);
    return this;
  }
  
  public final void l(Activity paramActivity)
  {
    GMTrace.i(19349901410304L, 144168);
    this.kLa = new WeakReference(paramActivity);
    GMTrace.o(19349901410304L, 144168);
  }
  
  public static abstract interface a
  {
    public abstract void i(k paramk);
  }
}


/* Location:              /Users/xianghongyan/decompile/dex2jar/classes2-dex2jar.jar!/com/tencent/mm/plugin/collect/reward/a/a.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */