  ...
  implements c
{
  public Plugin()
  {
    ...
    if (!ab.getContext().getSharedPreferences(ab.bNa() + "_google_aid", 4).getBoolean("already_report_googleaid", false)) {
      e.post(new Runnable()
      {
        public final void yv(String paramAnonymousString)
        {
          GMTrace.i(6823897726976L, 50842);
          ab.getContext().getSharedPreferences(ab.bNa() + "_google_aid", 4).edit().putString("getgoogleaid", paramAnonymousString).commit();
          paramAnonymousString = String.format("%s,%s,%s,%s,%s,%s", new Object[] { "", p.getDeviceID(ab.getContext()), p.getAndroidId(), p.tg(), paramAnonymousString, ab.getContext().getSharedPreferences(ab.bNa(), 0).getString("installreferer", "") });
          w.i("MicroMsg.Plugin.gai", "Advertisement MAT rsakv logID:%d val:%s", new Object[] { Integer.valueOf(11238), paramAnonymousString });
          g.paX.a(11238, paramAnonymousString, true, true);
          ab.getContext().getSharedPreferences(ab.bNa() + "_google_aid", 4).edit().putBoolean("already_report_googleaid", true).commit();
  ...
  {
    public abstract void yv(String paramString);
  }
}


/* Location:              /Users/xianghongyan/decompile/dex2jar/classes2-dex2jar.jar!/com/tencent/mm/plugin/gai/Plugin.class
